...
****************************************************************
      ...
      complex(dp) test1(2,2,2,2),test2(2,2,2,2)
      
c-----debug
      include 'kc_phasespace.f'
      epinv=0._dp
      scale=1._dp
      musq=1._dp
      call spinorz(6,p,za,zb)
      call amp_zaj_ql_assemble(1,2,3,4,5,6,za,zb,test1,test2)
!     test=amp_zaj_ql_slc_MHV(1,2,3,4,5,6,za,zb)
      stop
!---- end debug
...
c-----(-+xx-+)
     ...
     &   (+qq1*(q1*Q(qi)+r1*l(qi)*propzQL(3))*a60h(3,i)) 
      enddo
c-----Virtual amplitude
c-----Sum of nf light quark electric charge, coupling twice in loop
c----- weight by additional 1/Q(qi) to compensate overall coupling below
      SumQiNf=zip
      do i=1,nf!       FOR NOW: no top loop vector couplings
         SumQiNf=SumQiNf+Q(i)**2/Q(qi)
      enddo
c-----Sum of nf light quark z-coupling: vL(i)+vR(i) times photon coupling
c----- weight by additional 1/Q(qi) to compensate overall coupling below
      SumViNf=zip
      do i=1,nf!       FOR NOW: no top loop vector couplings
         SumViNf=SumViNf + (l(i)+r(i))*Q(i)/Q(qi)
      enddo
      a6vh(3,:)=0._dp ! DEBUG: no top or bottom loop axial couplings (all decays)
      a6vh(5,:)=0._dp ! DEBUG: no top or bottom loop axial couplings (electron decay)
!      a6vh(4:5,:)=0._dp ! DEBUG: no radiation from decay
c-----M(+-xx-+)
      do i=1,4
...
c-----square the helicity amplitudes
     ...
     &               +two*real(conjg(m60hB(i))*m6vhA(i))
      enddo
      do i=1,16
         m6sqvh(i)=m6sqvhAA(i)+m6sqvhBB(i)+m6sqvhAB(i)
!         if     (ipsgen == 1) then
!            m6sqvh(i)=m6sqvhAA(i)
