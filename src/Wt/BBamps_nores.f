...
C    . ,tsq
      ...
      tga2=2d0*gDt+cplx2(2d0*(aDt+aDg),zip)
      do j=1,4
      tvec(j)=p(it,j)
      bvec(j)=p(ib,j)
      enddo
      ...
      if (jpart==it) q(it,nu)=p(it,nu)-0.5D0*mtsq/nDt*p(in,nu)
      if (jpart==ib) q(ib,nu)=p(ib,nu)-0.5D0*mbsq/bDe*p(ie,nu)
      enddo
      enddo
...
c      tsq=dot(p,it,it)
c      gDt=gDt+cplx2((tsq-mtsq)/2d0 ,zip)  
...
c      if (real(2d0*gDt+mt**2) > zip) then
c        gDt=gDt+cplx2(zip,0.5d0*mt*twidth)
c      endif
