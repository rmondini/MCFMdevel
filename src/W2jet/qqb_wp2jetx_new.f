      subroutine qqb_wp2jetx_new(p,msq,mqq,ppmsqx,msqx_cs)
      implicit none
      include 'types.f'
c---- wm2jetx and wp2jetx should differ only by the include
c---- file:  ppwm2j.f  or  ppwp2j.f
c      
...
c
c--- Note: this differs from the "old" qqb_w2jetx.f by the fact
c---  that the array msqx with 3*11^4 = 43923 elements has been
c---  replaced by the array ppmsqx with 3*58 = 174 elements
c
      include 'constants.f'
      include 'nf.f'
      include 'mxpart.f'
...
c--- q(i) q(i) --> q(i) ( --> W q(j) ) q(i)
        ...
        qq_iiji(0)=2._dp/xn*real(qq1(1)*conjg(qq1(2)),dp)
...
c--- qb(i) qb(i) --> qb(i) ( --> W qb(j) ) qb(i)
        ...
        qbqb_iiji(0)=2._dp/xn*real(qbqb1(1)*conjg(qbqb1(2)),dp)
...
c--- Q QBAR - different flavours
     ...
     &            +facqq*(Vsum(k)-Vsq(j,k))*qqb_ijik(2)
           if ((j <= 4) .and. (k >= -4)) then
             do i=0,2
             tmpmsqx(i,pp(j,k,j,-j))=facqq*Vsq(j,k)*qqb_ijii(i)
             tmpmsqx(i,pp(j,k,-k,k))=facqq*Vsq(j,k)*qqb_ijjj(i)
             tmpmsqx(i,pp(j,k,-j,j))=facqq*Vsq(j,k)*qqbxijii(i)
             tmpmsqx(i,pp(j,k,k,-k))=facqq*Vsq(j,k)*qqbxijjj(i)
             enddo
             do l=1,4
             if ((l .ne. +j) .and. (l .ne. -k)) then
              tmpmsqx(1,pp(j,k,l,-l))=facqq*Vsq(j,k)*qqb_ijkk(1)
              tmpmsqx(1,pp(j,k,-l,l))=tmpmsqx(1,pp(j,k,l,-l))
             endif
             if (l .ne. +j) then
               tmpmsqx(2,pp(j,k,j,-l))=facqq*Vsq(k,l)*qqb_ijik(2)
               tmpmsqx(2,pp(j,k,-l,j))=facqq*Vsq(k,l)*qqbxijik(2)
             endif
             if (l .ne. -k) then
               tmpmsqx(2,pp(j,k,+l,k))=facqq*Vsq(j,-l)*qqb_ijkj(2)
               tmpmsqx(2,pp(j,k,k,+l))=facqq*Vsq(j,-l)*qqbxijkj(2)
             endif
             enddo
           endif
...
c--- Q QBAR - same flavours
     ...
     &             +facqq*Vsum(j)*qqb_iiji(2)
          if (j <= 4) then
            do l=1,4
            do i=0,2
            tmpmsqx(i,pp(j,k,j,-l))=facqq*Vsq(k,l)*qqb_iiij(i)
            tmpmsqx(i,pp(j,k,l,k))=facqq*Vsq(j,-l)*qqb_iiji(i)
            tmpmsqx(i,pp(j,k,-l,j))=facqq*Vsq(k,l)*qqbXiiij(i)
            tmpmsqx(i,pp(j,k,k,l))=facqq*Vsq(j,-l)*qqbXiiji(i)
            enddo
            do m=-4,1
              if ((l .ne. j) .and. (m .ne. k)) then
              tmpmsqx(1,pp(j,k,l,m))=facqq*Vsq(-l,-m)*qqb_ijkl(1)
              tmpmsqx(1,pp(j,k,m,l))=facqq*Vsq(-l,-m)*qqbXijkl(1)
              endif
            enddo
            enddo
          endif
...
c--- QBAR Q - different flavours
     ...
     &             +facqq*(Vsum(j)-Vsq(j,k))*qbq_ijik(2)
           if ((j >= -4) .and. (k <= 4)) then
             do i=0,2
             tmpmsqx(i,pp(j,k,-j,j))=facqq*Vsq(j,k)*qbq_ijjj(i)
             tmpmsqx(i,pp(j,k,k,-k))=facqq*Vsq(j,k)*qbq_ijii(i)
             tmpmsqx(sw(i),pp(j,k,j,-j))=facqq*Vsq(j,k)*qbqxijjj(i)
             tmpmsqx(sw(i),pp(j,k,-k,k))=facqq*Vsq(j,k)*qbqxijii(i)
             enddo
             do l=-4,-1
             if ((l .ne. +j) .and. (l .ne. -k)) then
              tmpmsqx(1,pp(j,k,l,-l))=facqq*Vsq(j,k)*qbq_ijkk(1)
              tmpmsqx(sw(1),pp(j,k,-l,l))=tmpmsqx(1,pp(j,k,l,-l))
             endif
             if (l .ne. +j) then
               tmpmsqx(2,pp(j,k,-l,j))=facqq*Vsq(k,l)*qbq_ijkj(2)
               tmpmsqx(sw(2),pp(j,k,j,-l))=facqq*Vsq(k,l)*qbqxijkj(2)
             endif
             if (l .ne. -k) then
               tmpmsqx(2,pp(j,k,k,+l))=facqq*Vsq(j,-l)*qbq_ijik(2)
               tmpmsqx(sw(2),pp(j,k,+l,k))=facqq*Vsq(j,-l)*qbqxijik(2)
             endif
             enddo
           endif
        else
c--- QBAR Q - same flavours
     ...
     &             +facqq*Vsum(k)*qbq_iiji(2)
          if (k <= 4) then
            do l=-4,-1
            do i=0,2
            tmpmsqx(i,pp(j,k,-l,j))=facqq*Vsq(k,l)*qbq_iiji(i)
            tmpmsqx(i,pp(j,k,k,l))=facqq*Vsq(j,-l)*qbq_iiij(i)
            tmpmsqx(sw(i),pp(j,k,j,-l))=facqq*Vsq(k,l)*qbqXiiji(i)
            tmpmsqx(sw(i),pp(j,k,l,k))=facqq*Vsq(j,-l)*qbqXiiij(i)
            enddo
            do m=1,4
              if ((l .ne. j) .and. (m .ne. k)) then
              tmpmsqx(1,pp(j,k,m,l))=facqq*Vsq(-l,-m)*qbq_ijkl(1)
              tmpmsqx(sw(1),pp(j,k,l,m))=facqq*Vsq(-l,-m)*qbqXijkl(1)
              endif
            enddo
            enddo
          endif
...
c--- Q Q - different flavours
     ...
     &          +facqq*half*Vsq(k,-j)*qq_ijii(2)
          if ((j <= 4) .and. (k <= 4)) then
            do i=0,2
            tmpmsqx(i,pp(j,k,j,j))=facqq*half*Vsq(k,-j)*qq_ijii(i)
            tmpmsqx(i,pp(j,k,k,k))=facqq*half*Vsq(j,-k)*qq_ijjj(i)
            enddo
            do l=-4,4
             if (l .ne. k) then
               tmpmsqx(1,pp(j,k,l,k))=facqq*Vsq(j,-l)*qq_ijkj(1)
               tmpmsqx(1,pp(j,k,k,l))=facqq*Vsq(j,-l)*qq_ijjk(1)
             endif
             if (l .ne. j) then
               tmpmsqx(1,pp(j,k,j,l))=facqq*Vsq(k,-l)*qq_ijik(1)
               tmpmsqx(1,pp(j,k,l,j))=facqq*Vsq(k,-l)*qq_ijki(1)
             endif
            enddo
          endif
        else
c--- Q Q - same flavours
          ...
          if (j <= 4) then
            do l=-4,4
              ...
              do i=0,2
              tmpmsqx(i,pp(j,k,l,j))=facqq*Vsq(j,-l)*qq_iiji(i)
              tmpmsqx(i,pp(j,k,j,l))=facqq*Vsq(j,-l)*qq_iiij(i)
              enddo
              endif
            enddo
          endif
...
c--- QBAR QBAR - different flavours
     ...
     &     +facqq*half*Vsq(k,-j)*qbqb_ijii(2)
          if ((j >= -4) .and. (k >= -4)) then
            do i=0,2
            tmpmsqx(i,pp(j,k,j,j))=facqq*half*Vsq(k,-j)*qbqb_ijii(i)
            tmpmsqx(i,pp(j,k,k,k))=facqq*half*Vsq(j,-k)*qbqb_ijjj(i)
            enddo
            do l=-4,4
             if (l .ne. k) then
               tmpmsqx(1,pp(j,k,l,k))=facqq*Vsq(j,-l)*qbqb_ijkj(1)
               tmpmsqx(1,pp(j,k,k,l))=facqq*Vsq(j,-l)*qbqb_ijjk(1)
             endif
             if (l .ne. j) then
               tmpmsqx(1,pp(j,k,j,l))=facqq*Vsq(k,-l)*qbqb_ijik(1)
               tmpmsqx(1,pp(j,k,l,j))=facqq*Vsq(k,-l)*qbqb_ijki(1)
             endif
            enddo
          endif
        else
c--- QBAR QBAR - same flavours
          ...
          if (j >= -4) then
            do l=-4,4
              if (Vsq(j,-l) .ne. 0._dp) then
              do i=0,2
              tmpmsqx(i,pp(j,k,l,j))=facqq*Vsq(j,-l)*qbqb_iiji(i)
              tmpmsqx(i,pp(j,k,j,l))=facqq*Vsq(j,-l)*qbqb_iiij(i)
              enddo
              endif
            enddo
          endif
...
c--- 2-quark, 2-gluon contribution to matrix elements      
      ...
      if     ((j > 0) .and. (k < 0)) then
          ...
          do i=0,2
            ...
            if ((j <= 4) .and. (k >= -4)) then
              tmpmsqx(i,pp(j,k,0,0))=Vsq(j,k)*qqbWgg2_cs(i)
            endif
          enddo
      elseif ((j < 0) .and. (k > 0)) then
          ...
          do i=0,2
            ...
            if ((j >= -4) .and. (k <= 4)) then
              tmpmsqx(i,pp(j,k,0,0))=Vsq(j,k)*qbqWgg2_cs(i)
            endif
          enddo
      elseif ((j > 0) .and. (k == 0)) then
          msq(j,k)=msq(j,k)+
     ...
     & (Vsq(j,-1)+Vsq(j,-2)+Vsq(j,-3)+Vsq(j,-4)+Vsq(j,-5))*qgWqg2_cs(i)
            if (j <= 4) then
              do n1=1,4
              tmpmsqx(i,pp(j,k,n1,0))=Vsq(j,-n1)*qgWqg2_cs(i)
              tmpmsqx(i,pp(j,k,0,n1))=Vsq(j,-n1)*qgWgq2_cs(i)
              enddo
            endif
           enddo
      elseif ((j < 0) .and. (k == 0)) then
     ...
     & (Vsq(j,+1)+Vsq(j,+2)+Vsq(j,+3)+Vsq(j,+4)+Vsq(j,+5))*qbgWqbg2_cs(i)
            if (j >= -4) then
              do n1=1,4
              tmpmsqx(i,pp(j,k,-n1,0))=Vsq(j,n1)*qbgWqbg2_cs(i)
              tmpmsqx(i,pp(j,k,0,-n1))=Vsq(j,n1)*qbgWgqb2_cs(i)
              enddo
            endif
          enddo
      elseif ((j == 0) .and. (k > 0)) then
     ...
     & (Vsq(-1,k)+Vsq(-2,k)+Vsq(-3,k)+Vsq(-4,k)+Vsq(-5,k))*gqWqg2_cs(i)
            if (k <= 4) then
              do n1=1,4
              tmpmsqx(i,pp(j,k,n1,0))=Vsq(-n1,k)*gqWqg2_cs(i)
              tmpmsqx(i,pp(j,k,0,n1))=Vsq(-n1,k)*gqWgq2_cs(i)
              enddo
            endif
          enddo
      elseif ((j == 0) .and. (k < 0)) then
     ...
     &(Vsq(+1,k)+Vsq(+2,k)+Vsq(+3,k)+Vsq(+4,k)+Vsq(+5,k))*gqbWqbg2_cs(i)
            if (k >= -4) then
              do n1=1,4
              tmpmsqx(i,pp(j,k,-n1,0))=Vsq(n1,k)*gqbWqbg2_cs(i)
              tmpmsqx(i,pp(j,k,0,-n1))=Vsq(n1,k)*gqbWgqb2_cs(i)
              enddo
            endif
           enddo
      elseif ((j == 0) .and. (k == 0)) then
          ...
          do n1=1,4
            do n2=-4,-1
              Vfac=Vfac+Vsq(n1,n2)
              do i=0,2
              tmpmsqx(i,pp(j,k,-n1,-n2))=Vsq(n1,n2)*ggWqbq2_cs(i)
              tmpmsqx(i,pp(j,k,-n2,-n1))=Vsq(n1,n2)*ggWqqb2_cs(i)
              enddo
            enddo
          enddo
