...
! Catch improper usage
      ...
      call fdist_select(ih,ibeam,x+h,facscale,pdfup)
      call fdist_select(ih,ibeam,x-h,facscale,pdfdn)
      pdfup_save(1,:)=pdfup(:)
      pdfdn_save(1,:)=pdfdn(:)
...
! Loop over successively smaller stepsizes and higher orders of extrapolation
      do i=2,NTAB
        ...
        if (calledpdf(i) .eqv. .false.) then
          call fdist_select(ih,ibeam,x+hh,facscale,pdfup)
          call fdist_select(ih,ibeam,x-hh,facscale,pdfdn)
          pdfup_save(i,:)=pdfup(:)
          pdfdn_save(i,:)=pdfdn(:)
          calledpdf(i)=.true.
        endif
