      subroutine fdist_select(ih_call,ibeam,xx,xxmu,ffx)
...
! pdfs according to selectpdfs(ibeam), passed via common block
     ...
     & CT14Pdf
      ...
      integer mode,Iprtn,ih,ibeam,iZ,iA,Irt
...
c--- explicitly set some pdfs to zero according to selectpdfs
      ...
      elseif (selectpdfs(ibeam) == +1) then
        ffx(-5:0)=0d0  ! remove antiquark and gluon pdfs
      elseif (selectpdfs(ibeam) == -1) then
        ffx(0:5)=0d0  ! remove quark and gluon pdfs
      endif
