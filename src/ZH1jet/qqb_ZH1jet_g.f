...
c--all momenta incoming
      ...
      include 'nflav.f'
      include 'hbbparams.f'
      integer:: i,j,k,pq,pl,nquark,nup,ndo,j1,j2,j3,j4,icol
      integer,parameter::swap(2)=(/2,1/),swap1(0:2)=(/0,2,1/)
      real(dp):: msq(-nf:nf,-nf:nf),p(mxpart,4),fac,faclo,s56,s1278,
...
c    .   qbqZgg2(2,2),qbgZqbg2(2,2),gqbZqbg2(2,2),
     ...
     &   ggZqbq2_cs(0:2,2,2),ggtemp(0:2)
      real(dp):: tup,tdo,msqhtautau,msqhbb,msqhgamgam,hdecay,facdecay,s4
      real(dp):: sH
...
!$omp threadprivate(/mqq/)
     ...
     & +s(j2,j3)+s(j2,j4)+s(j3,j4)
      ...
      call spinoru(n6,p,za,zb)
      

      s1278=s4(1,2,n5,n6)
      prop=cone/cplx2((s(3,4)-zmass**2),zmass*zwidth)
      prop=s(3,4)*prop/cplx2((s1278-zmass**2),zmass*zwidth)
...
c--- calculate 2-quark, 2-gluon amplitudes
      if (Gflag) then
        call z2jetsq(1,2,3,4,n5,n6,za,zb,qqbZgg2)
        call storecsz(qqbZgg2_cs)
        call z2jetsq(1,n5,3,4,2,n6,za,zb,qgZqg2)
        call storecsz(qgZqg2_cs)
        call z2jetsq(2,n5,3,4,1,n6,za,zb,gqZqg2)
...
C --NB this is the matrix element for gg->Z qb(5) q(6)
        call z2jetsq(n5,n6,3,4,1,2,za,zb,ggZqbq2)
...
c--- qRb->qRb
      call ampqqb_qqb(1,n5,2,n6,qRb_a,qRb_b)
c--- qR->qR
c instead of calling ampqqb_qqb(1,5,6,2,qR_a,qR_b)
      ...
      enddo
c--- qbR->qbR
      call ampqqb_qqb(n6,1,n5,2,qbR_a,qbR_b)
...
c--- qqb->qqb
      call ampqqb_qqb(1,2,n5,n6,qqb_a,qqb_b)
c--- qbq->qqb
...
c--- qq->qq
      call ampqqb_qqb(1,n6,n5,2,qq_a,qq_b)
