...
c---   p_phys: array of momenta to evaluate integrated dipoles
      ...
      subroutine qqb_fourgam_fragdips(p,p_phys,qcd_tree,msq_out) 
      implicit none
      include 'types.f'
      include 'constants.f'
      include 'nf.f'
...
c--- fill output array
      ...
      do k=-nf,nf
         ...
         if    ((j==0).and.(k.ne.0)) then
            msq_out(j,k)=2d0*msq_qcd(j,k)*Q(k)**2*virt_dip
         elseif((j.ne.0).and.(k==0)) then
            msq_out(j,k)=2d0*msq_qcd(j,k)*Q(j)**2*virt_dip
         endif
