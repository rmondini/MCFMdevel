...
c      s2max=min(s1,bbsqmax)
     ...
     &   .or. (kcase==kW_cwdk) .or. (kcase==kWtbwdk)
     &   .or. (kcase==kqq_tth)) then
        oldzerowidth=zerowidth
        zerowidth=.true.
      elseif ((kcase==kqq_ttw)
     &   .or. (kcase==kWttmas)) then
        s2min=4._dp*mt**2
      elseif ((kcase==kdm2jet).or.(kcase==kdm_gaj)) then 
         s2min=4._dp*xmass**2
      elseif ((kcase==kdm_jet).or.(kcase==kdm_gam)) then 
         s2min=4._dp*xmass**2
      elseif ((kcase==kWWqqbr).or.(kcase==kWZbbar)) then 
         s2min=bbsqmin
     ...
     &   .or. (kcase==kWH1jet) .or. (kcase==kZH1jet)) then
      ...
      if (kcase==kttwldk) then
        if (abs(m1-mt) < 1.e-6_dp) then
          s2min=mb**2       ! when called from gen9dk_rap
        else
          s2min=4._dp*mt**2   ! when called from gen4/phase4
        endif
      endif
...
!         if(kcase.ne.kZHbbnn) then 
!         write(6,*) n2,mass2,s2min,s2max,s2,width2         
!         pause
!      endif
...
c      s3min=mt**2 ! DEBUG: hack for s46 small
      if ((kcase==kqq_ttz) .or. (kcase==kqqtthz)) s3min=wsqmin
      s3max=(m2-m1)**2
...
c      if (s3max-s3min < 1.e-9_dp) return 1
      ...
      if (s3 < 1.e-14) return 1 ! for safety
