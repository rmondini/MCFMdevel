...
c            pbreak_part=pbreak
      ...
      if (kcase==kdirgam) ptmin_part=gammpt/three
...
c--- for gamma+2 jets process, permute jets (up to 3 for real)
      if (kcase == kgam_2j) then
        if     (njets == 2) then
          ...
          tmp(4,:)=p(4,:)  
          tmp(5,:)=p(5,:)  
          p(4,:)=tmp(1+perm2_3(perm),:)
          p(5,:)=tmp(1+perm2_4(perm),:)
        elseif (njets == 3) then
          ...
          tmp(5,:)=p(5,:)  
          tmp(6,:)=p(6,:)  
          p(4,:)=tmp(1+perm3_3(perm),:)
          p(5,:)=tmp(1+perm3_4(perm),:)
          p(6,:)=tmp(1+perm3_5(perm),:)
        else
          write(6,*)' Wrong number of jets in gen_photons_jets.f: ',njets
          stop
        endif
      endif
      
c--- for gamma+3 jets process, permute jets
      if (kcase == kgam_3j) then
          perm=int(r(3*nphotsjets-1)*6._dp)
          if (perm < 0) perm=0
          if (perm > 5) perm=5
          tmp(4,:)=p(4,:)  
