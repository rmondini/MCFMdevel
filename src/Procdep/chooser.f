...
c-----------------------------------------------------------------------
      ...
      elseif ((nproc >= 41) .and. (nproc <= 43)) then
        ...
        mcfmplotinfo= (/ 34, 345, 56, 3456, (0,j=1,46) /)
...
c--    '  f(p1)+f(p2) --> Z^0 (no BR) +f(p5)+f(p6)' (removebr=.true.)
        ...
        if (rank == 0) write(6,*)
c--- runstring controls treatment of top loops
        if    (index(runstring,'topapprox') > 0) then
          toploops=tapprox
          if (rank == 0) write(*,*) 'Including approximate treatment of top loops'
        elseif (index(runstring,'topexact') > 0) then
          toploops=texact
          if (rank == 0) write(*,*) 'Including exact treatment of top loops'
        else
          toploops=tnone
          if (rank == 0) write(*,*) 'Omitting top-quark loop contributions'
        endif

c--- runstring controls whether various top-loop pieces should be included
c--- [if none are specified, default is to include all]
        toplight=.false.
        topvector=.false.
        topaxial=.false.
        
        if (index(runstring,'L') > 0) toplight=.true.
        if (index(runstring,'V') > 0) topvector=.true.
        if (index(runstring,'A') > 0) topaxial=.true.

        if   ((toplight .eqv. .false.) .and. (topvector .eqv. .false.)
     &  .and. (topaxial .eqv. .false.)) then
          toplight=.true.
          topvector=.true.
          topaxial=.true.
        endif

        if (toploops /= tnone) then
          if (toplight .and. (rank == 0)) write(*,*)  '  top-quark loops with light-Z coupling included'
          if (topvector .and. (rank == 0)) write(*,*) '  top-Z vector coupling included'
          if (topaxial .and. (rank == 0))  write(*,*) '  top-Z axial coupling included'
        endif
...
c--- total cross-section             
      ...
      elseif (nproc == 49) then
c-- 49 '  f(p1)+f(p2) --> Z^0(-->e^-(p3)+e^+(p4))+f(p5)+f(p6) [fermion loop]'
c--    '  f(p1)+f(p2) --> Z^0 (no BR) +f(p5)+f(p6)' (removebr=.true.)
        kcase=kZ2jetF
        call checkminzmass(1)
        ndim=10
...
c-----------------------------------------------------------------------
     ...
     &    .or. (nproc==1011) .or. (nproc==1012)
     &    .or. (nproc==1013) .or. (nproc==1014)
     &    .or. (nproc==110) .or. (nproc==1010)) then
         kcase=kZHbbar
         if(nproc.eq.1010) kcase=kZHbbdk
         if(nproc.eq.1011) kcase=kZHbbnn
         if(nproc.eq.1012) kcase=kZHbbjt
...
!------- setup code for massless decays (mb_eff set in hparams)
        ...
        plabel(6)='ba'
        plabel(7)='pp'
        if((nproc.eq.1013).or.(nproc.eq.1014)) coeffonly=.true.
        if((nproc.eq.1014)) usescet=.true.
        if(nproc.eq.1012) plabel(8)='pp'
        if(nproc.eq.1014) plabel(8)='pp'
        notag=2

        ndim=10
        if(nproc.eq.1012) ndim=13
        if(nproc.eq.1014) ndim=13
        nwz=0 
        n2=1
        n3=1
        mass2=hmass
        ...
        if ((nproc == 101).or.(nproc==1010)
     &       .or.(nproc==1011).or.(nproc==1012)
     &       .or.(nproc==1013).or.(nproc==1014)) then
...
c-----------------------------------------------------------------------
      ...
      elseif ((nproc == 280) .or. (nproc == 281)) then
             ndim=4 
             reset_alphaEM=.true.
             plabel(3)='ga'
             plabel(4)='pp'
             ...
             if (nproc == 280) then
                ...
                if (rank == 0) then
                write(6,*)
                write(6,*) 'Setting inclusive = .true. for direct photon production.'
                endif
             else
               kcase=kgamjet
             endif
  
      elseif ((nproc == 282).or.(nproc==2820)) then
             ndim=7 
             kcase=kgam_2j
             reset_alphaEM=.true.
             if(usescet.and.(nproc==2820)) then
!------- 2820 will allow jets to be integrated out for inc. photon (dirgam)
                scet_inc_jets=.true.
             endif
             plabel(3)='ga'
             plabel(4)='pp'
...
c-----------------------------------------------------------------------
       ...
       elseif (nproc == 701) then
        kcase=kHgagaI
!--- flag to use HEFT or not in LO calculation
        if (kpart == klord) then
          useHEFT=.false. ! may be set to T if desired
        else
          useHEFT=.true.  ! must be set to T for consistency
        endif
!--- flags for Re/Im parts of interference
        propimag=.true.
        propreal=.false.
        if (index(runstring,'Imag') > 0) then
          propimag=.true.
          propreal=.false.
        endif
        if (index(runstring,'Real') > 0) then
          propimag=.false.
          propreal=.true.
        endif
!--- diagnostic flag (for checking |H|^2 contributions)
        if (index(runstring,'Hsq') > 0) then
          checkHsq=.true.
        else
          checkHsq=.false.
        endif
...
c--  701 '  f(p1)+f(p2) --> H(-->gamma^-(p3)+gamma^+(p4)) [including interference]'
        plabel(3)='ga'
        plabel(4)='ga'
        hdecaymode='gaga'
        nqcdjets=0
...
!      if(reset_alphaEM) call reset_aem(aem)
      if(reset_alphaEM) call reset_aem(1._dp/137.035999074_dp)
...
c--- set flags to true unless we're doing W+2 jet or Z+2 jet
     ...
     &.and. (kcase.ne.kgam_2j) .and. (kcase.ne.kgam_3j))
     & .or. (kpart==klord) ) then
        Qflag=.true.
        Gflag=.true.
      endif
