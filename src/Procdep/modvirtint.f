...
c---- SSend 
      ...
      integer itrial
      real(dp):: savescale,savefacscale,xmsqup,xmsqdn,alphas
...
! compute weights for scale variation, looping if necessary
      if (doscalevar) then
        if (itrial == 3) xmsqdn=xmsq
        if (itrial == 2) xmsqup=xmsq
        itrial=itrial-1
        if (itrial > 0) goto 66
        if (abs(xmsq) > zip) then
          scalereweightdn=xmsqdn/xmsq
          scalereweightup=xmsqup/xmsq
        else
          scalereweightdn=zip
          scalereweightup=zip
        endif
...
c--- return both to .true. and assign value to virtint (to return to VEGAS)
      ...
      elseif (nshot == 2) then
        ...
        if (abs(xmsq_old/virtint-1._dp) > 1.e-8_dp) then
           if (rank == 0) then
            write(6,*) 'epsilon fails to cancel'
            write(6,*) 'xmsq (epinv=large) = ',xmsq_old
            write(6,*) 'xmsq (epinv=zero ) = ',virtint
            write(6,*) 'fractional difference',virtint/xmsq_old-1d0
            call flush(6)
         endif
C          stop
          colourchoice=rvcolourchoice
