...
c--- To use VEGAS random number sequence :
     ...
     & fxb1(-nf:nf),fxb2(-nf:nf),xmsq_array(-nf:nf,-nf:nf)
      real(dp):: wgt,msq(-nf:nf,-nf:nf),m3,m4,m5,xmsqjk,reweightEWvjet
     ...
     & msq4(-nf:nf,-nf:nf,-nf:nf,-nf:nf)
      ...
      complex(dp):: ampVec(2,2,2,2),ampAx(0:2,2,2,2,2)
     ...
     & qqb_Zbbmas,qqb_Zbbmas,qqb_totttZ,qqb_totttZ_mad
      ...
      external qqb_gamgam_g,qqb_gmgmjt_gvec,qqb_dirgam_g,qqb_gam2j,qqb_gam2j_gvec
...
c        enddo
c        pause
        if (index(runstring,'EW') > 0) then
          msq=msq*reweightEWvjet(p) ! apply EW corrections
        endif
      elseif (kcase==kZ_2jet) then
        call qqb_z2jet(p,msq)
      elseif (kcase==kZ2jetF) then
         call qqb_z2jet_floop(p,msq)
...
c        call qqb_3jet(p,msq)
      elseif ((kcase==kdirgam) .or. (kcase==kgamjet)) then
        ...
        if (index(runstring,'EW') > 0) then
          call qqb_dirgam_ew(p,msq1)
          msq=msq1-msq
        endif
!        do j=-nf,nf
!        do k=-nf,nf
!        if (msq(j,k) .ne. 0._dp) then
!          write(6,*) j,k,msq(j,k),msq1(j,k)/msq(j,k)
!        endif
!        enddo
!        enddo
!        pause
...
!        do k=-nf,nf
!        if (j == 0) then
!          call checkgvec(j,k,1,p,qqb_gam2j,qqb_gam2j_gvec)
!        endif
!        if (k == 0) then
!          call checkgvec(j,k,2,p,qqb_gam2j,qqb_gam2j_gvec)
!        endif
!        if (((j == -k) .and. abs(j)>0) .or. (j*k == 0) .and. (j-k .ne. 0)) then
!          call checkgvec(j,k,4,p,qqb_gam2j,qqb_gam2j_gvec)
!          call checkgvec(j,k,5,p,qqb_gam2j,qqb_gam2j_gvec)
!        endif
!        enddo
!        enddo
!        pause
...
!$omp critical(critPDFerrors)
            ...
            call fdist_select(ih1,1,xx(1),facscale_H,fx1_H)
            call fdist_select(ih2,2,xx(2),facscale_H,fx2_H)
            call fdist_select(ih1,1,xx(1),facscale_L,fx1_L)
            call fdist_select(ih2,2,xx(2),facscale_L,fx2_L)
...
!$omp critical(critPDFerrors)
                call InitPDF(currentPDF)
                call fdist_select(ih1,1,xx(1),b1scale,fxb1)
                call fdist_select(ih2,2,xx(2),q2scale,fx2)
                call fdist_select(ih1,1,xx(1),q1scale,fx1)
                call fdist_select(ih2,2,xx(2),b2scale,fxb2)
!$omp end critical(critPDFerrors)
             else
                call fdist_select(ih1,1,xx(1),b1scale,fxb1)
                call fdist_select(ih2,2,xx(2),q2scale,fx2)
...
!$omp critical(critPDFerrors)
               call InitPDF(currentPDF)
               call fdist_select(ih1,1,xx(1),facscale,fx1)
               call fdist_select(ih2,2,xx(2),facscale,fx2)
!$omp end critical(critPDFerrors)
             else
               call fdist_select(ih1,1,xx(1),facscale,fx1)
               call fdist_select(ih2,2,xx(2),facscale,fx2)
...
!$omp end critical(critPDFerrors)
              ...
              call fdist_select(ih2,2,xx(2),facscale,fx2)
              if (doscalevar) then
                call fdist_select(ih1,1,xx(1),facscale*two,fx1up)
                call fdist_select(ih2,2,xx(2),facscale*two,fx2up)
                call fdist_select(ih1,1,xx(1),facscale/two,fx1dn)
                call fdist_select(ih2,2,xx(2),facscale/two,fx2dn)
                xmsqvar(:)=zip
              endif
           endif
        endif
