...
c        call gen_photons_jets(r,2,1,p,pswt,*999)
        if     ((usescet) .and. (abovecut) .and. (kpart.ne.kreal) .and. (doQT .eqv. .false.)) then
          if (tauboost) then
...
c--- special treatment for Z+gamma+jet+jet 
      ...
      elseif ((kcase==kgamjet)) then
        call gen2jet(r,p,pswt,*999)
!        call gen_photons_jets(r,1,1,p,pswt,*999)
        npart=2

      elseif ((kcase==kthrjet) .or. (kcase==kgam_2j)) then
...
!        endif
      ...
      elseif (kcase==kgam_3j) then
        call gen_photons_jets(r,1,3,p,pswt,*999)
        npart=4
...
c--- processes that use "gen_njets" with an argument of "1"     
     ...
     &    .or. (kcase==kgQ__ZQ) ) then
        ...
        elseif ((usescet) .and. (abovecut) .and. (kpart == kreal) .and. (doQT .eqv. .false.)) then
            call gen3(r,p,pswt,*999)
         else
...
c--- WH processes with 2-body Higgs decays
     ...
     &    .or. (kcase==kZHbbnn)  .or. (kcase==kZHn2bc)
     &    .or. (kcase==kWHgaga)  .or. (kcase==kZHgaga) ) then
        npart=4
        call genVH(r,p,pswt,*999)


!---- phase space for V (H=>bbg)
      elseif( (kcase==kZHbbjt).or.(kcase==kZHn2ac)) then
         npart=5
         call gen5(r,p,pswt,*999) 
