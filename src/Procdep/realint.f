...
cz //
      ...
      integer itrial
      real(dp):: alphas,msqtrial,fac1,fac2,
...
c--- test to see whether we need Gflag and Qflag together
      if ( ((kcase==kW_2jet) .or. (kcase==kZ_2jet) .or. (kcase==kGam_2j))
...
c--- (W+2 jet, Z+2 jet and gamma+2 jet processes only)
   44 continue   
...
c        call singcheck(qqb_zh_g,qqb_zh_gs,p)
      ...
      elseif (kcase==kZHbbnn) then
         if (includereal) call qqb_zh_bbg(p,msq)
         call qqb_zh_bb_gs(p,msqc)
      elseif (kcase==kZHn2bc) then
       if (includereal) call qqb_zh_bbg_nlopxd_bc(p,msq)
        call qqb_zh_bb_nlopxd_bc_gs(p,msqc)
      elseif (kcase==kZHn2ac) then
       if (includereal) call qqb_zh_bbg_nlopxd_ac(p,msq)
        call qqb_zh_bb_nlopxd_ac_gs(p,msqc)
      elseif (kcase==kZHbbjt) then
         if (includereal) call qqb_zh_bbg_g(p,msq)
         call qqb_zh_bbg_gs(p,msqc)
...
!      p(  1,  2) =    0.00000000000000 _dp   
!      p(  1,  3) =   -127.937968910860_dp    
!      p(  1,  4) =   -127.937968910860   _dp 
!      p(  2,  1) =    0.00000000000000    _dp
!      p(  2,  2) =    0.00000000000000    _dp
!      p(  2,  3) =    465.525429304868    _dp
!      p(  2,  4) =   -465.525429304868    _dp
!      p(  3,  1) =   -145.063526064082    _dp
!      p(  3,  2) =   -48.2648875533012    _dp
!      p(  3,  3) =   0.794340595662277    _dp
!      p(  3,  4) =    152.884129135955    _dp
!      p(  4,  1) =    114.657835009406    _dp
!      p(  4,  2) =    67.2982228391552    _dp
!      p(  4,  3) =   -369.348376429546    _dp
!      p(  4,  4) =    392.547695315479    _dp
!      p(  5,  1) =   0.122693249233876    _dp
!      p(  5,  2) =   0.695876624442816E-01_dp
!      p(  5,  3) =  -0.404089176341461    _dp
!      p(  5,  4) =   0.428000161925607    _dp
!      p(  6,  1) =    30.2829978054421    _dp
!      p(  6,  2) =   -19.1029229482983    _dp
!      p(  6,  3) =    31.3706646162161    _dp
!      p(  6,  4) =    47.6035736023689    _dp
!      p(  1,  1) =    0.00000000000000    _dp
!      p(  1,  2) =    0.00000000000000    _dp
!      p(  1,  3) =   -785.985255337572    _dp
!      p(  1,  4) =   -785.985255337572    _dp
!      p(  2,  1) =    0.00000000000000    _dp
!      p(  2,  2) =    0.00000000000000    _dp
!      p(  2,  3) =    455.523430319579    _dp
!      p(  2,  4) =   -455.523430319579    _dp
!      p(  3,  1) =    291.461337231822    _dp
!      p(  3,  2) =   -151.896204402546    _dp
!      p(  3,  3) =   -103.834185331600    _dp
!      p(  3,  4) =    344.679134930356    _dp
!      p(  4,  1) =   -48.6130184974871    _dp
!      p(  4,  2) =    37.8745880250802    _dp
!      p(  4,  3) =    585.545834172565    _dp
!      p(  4,  4) =    588.779783877089    _dp
!      p(  5,  1) =   -242.843615732266    _dp
!      p(  5,  2) =    114.018378497540    _dp
!      p(  5,  3) =   -151.298519498574    _dp
!      p(  5,  4) =    308.000737563385    _dp
!      p(  6,  1) =  -0.470300206881682E-02_dp
!      p(  6,  2) =   0.323787992506073E-02_dp
!      p(  6,  3) =   0.486956756017297E-01_dp
!      p(  6,  4) =   0.490292863213131E-01_dp
!      includereal=includedipole(0,p)
!      incldip(0)=includereal 
!        call scaleset(initscale,initfacscale,p)
!        dipscale(0)=facscale
...
!         do j=1,6
!         do k=j+1,6
!         if (abs(s(j,k)) < ptmp) ptmp=abs(s(j,k))
!         enddo
!         enddo
!         if (ptmp < 1.e-2) then
!           call writeout(p)
!           call singcheck(qqb_gam2j_g,qqb_gam2j_gs,p)
!!           pause
!         endif
!         endif
!        if (includereal) call singcheck(qqb_gam2j_g,qqb_gam2j_gs,p)
        if (includereal) call qqb_gam2j_g(p,msq)
        call qqb_gam2j_gs(p,msqc)
!        call qqb_gam2j_g_mad(p,msqa)
!        call qqb_gam2j_g(p,msq)
!        do j=-4,4
!        do k=-4,4
!        if (msq(j,k) .ne. zip) write(6,*) j,k,msq(j,k),msq(j,k)/msqa(j,k)
!        enddo
!        enddo
!        stop
...
!        call singcheck(gg_Hgaga_Hpi_g,gg_Hgaga_Hpi_gs,p)
!        if (includereal) call gg_Hgaga_Hpi_g(p,msq)
!        call gg_Hgaga_Hpi_gs(p,msqc)
...
c--- and set all PDF entries to zero
              ...
              call fdist_select(ih1,1,xx1,dipscale(nd),fx1)
              call fdist_select(ih2,2,xx2,dipscale(nd),fx2)
              dipfx1(nd,:)=fx1(:)
              dipfx2(nd,:)=fx2(:)
            endif
          enddo
...
c--- for single top + b, make sure to use two different scales
            call InitPDF(currentPDF)
            call fdist_select(ih1,1,xx1,facscale_H,fx1_H)
            call fdist_select(ih2,2,xx2,facscale_H,fx2_H)
            call fdist_select(ih1,1,xx1,facscale_L,fx1_L)
            call fdist_select(ih2,2,xx2,facscale_L,fx2_L)
            do j=-nf,nf
            ...
            call InitPDF(currentPDF)
            call fdist_select(ih1,1,xx1,facscale,fx1)
            call fdist_select(ih2,2,xx2,facscale,fx2)
          endif
        endif
...
c--- and set all PDF entries to zero
              ...
              if (doscalevar) then
                call fdist_select(ih1,1,xx1,dipscale(nd)*two,fx1)
                call fdist_select(ih2,2,xx2,dipscale(nd)*two,fx2)
                dipfx1up(nd,:)=fx1(:)
                dipfx2up(nd,:)=fx2(:)
                call fdist_select(ih1,1,xx1,dipscale(nd)/two,fx1)
                call fdist_select(ih2,2,xx2,dipscale(nd)/two,fx2)
...
c--- usual case            
            ...
            call fdist_select(ih2,2,xx2,facscale,fx2)
            if (doscalevar) then
              call fdist_select(ih1,1,xx1,facscale*two,fx1up)
              call fdist_select(ih2,2,xx2,facscale*two,fx2up)
              call fdist_select(ih1,1,xx1,facscale/two,fx1dn)
              call fdist_select(ih2,2,xx2,facscale/two,fx2dn)
              xmsqvar(:,:)=zip
            endif
          endif
        endif
...
! compute weights for scale variation
      if (doscalevar) then
        if (abs(xmsq(nd)) > zip) then
          ...
          fac1=(alphas(scaleup,amz,nlooprun)/asorig)**alphaspow
          fac2=(alphas(scaledn,amz,nlooprun)/asorig)**alphaspow
          scalereweight(1)=fac1*xmsqvar(1,nd)*flux*pswt/BrnRat/xmsq(nd)
          scalereweight(2)=fac2*xmsqvar(2,nd)*flux*pswt/BrnRat/xmsq(nd)
          if (maxscalevar == 6) then
            scalereweight(3)=fac1
            scalereweight(4)=fac2
...
!$omp critical(MaxWgt)
         wtmax=abs(valsum)
!         if(wtmax.gt.0.5_dp) then
!            call writeout(p)
!            do j =-nf,nf
!               do k=-nf,nf
!                  write(6,*) j,k,msq(j,k),msqc(:,j,k)
!               enddo
!            enddo
!            write(6,*) 'new max weight > 0.5',wtmax
!            do j=1,ndim+2
!               write(6,*) '     vector(',j,')=',vector(j),'._dp'
!            enddo
!            pause
!         endif
...
!        write(6,*) 'p1ext',p1ext
!        write(6,*) 'p2ext',p2ext
!        write(6,*) 'xx1,xx2',xx1,xx2
!        do j=-nf,nf
!        do k=-nf,nf
!        write(6,*) j,k,msq(j,k),msqc(1:12,j,k)
!        enddo
!        enddo
!        write(6,*) 'stopped with return value of realint NaN'
c        stop
