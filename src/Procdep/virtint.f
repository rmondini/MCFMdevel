...
c--- contributions, so all these should be set to zero
     ...
     & .or. (kcase==kZHbbjt) .or. (kcase==kZHbbnn)
...
c--- correct for scaling with NLO partial width
      ...
      elseif (kcase==kZHbbjt) then
        ...
        call qqb_zh_bbg_v(p,msqv)
!        call qqb_zh_bbg_z(p,z)
      elseif (kcase==kZHbbnn) then
        call qqb_zh(p,msq)
        call qqb_zh_bb_v(p,msqv)
!     call qqb_zh_bbg_z(p,z)
      elseif (kcase==kZHn2bc) then
         call qqb_zh_bb_nlopxd_bc(p,msq)
         call qqb_zh_bb_nlopxd_bc_v(p,msqv)
         call qqb_zh_z(p,z)
      elseif (kcase==kZHn2ac) then
         call qqb_zh_bb_nlopxd_ac(p,msq)
         call qqb_zh_bb_nlopxd_ac_v(p,msqv)
         call qqb_zh_z(p,z)
 ...
 !        call qqb_dirgam_vbis(p,msqvdk,1)
!         do j=-nf,nf
!        do k=-nf,nf
!        msqvdk(j,k)=msqvdk(j,k)-(pisq/twelve*(2._dp*CF+CA)-0*11._dp/6._dp)
!     &  *ason2pi*msq(j,k)
!        if (msqv(j,k) .ne. 0._dp) then
!        write(6,*) '1L',j,k,msqv(j,k),msqvdk(j,k),(msqvdk(j,k)-msqv(j,k))
!     &   /msq(j,k)/ason2pi
!        endif
!        enddo
!        enddo
...
! checking code
!        call gg_hg_vbis(p,msqvdk,1)
!        do j=-nf,nf
!        do k=-nf,nf
!        if ((j == 0) .and. (k == 0)) then
!          msqvdk(j,k)=msqvdk(j,k)-(pisq/twelve*(3._dp*CA))*ason2pi*msq(j,k)
!        else
!          msqvdk(j,k)=msqvdk(j,k)-(pisq/twelve*(2._dp*CF+CA))*ason2pi*msq(j,k)
!        endif
!        if (msqv(j,k) .ne. 0._dp) then
!        write(6,*) '1L',j,k,msqv(j,k),msqvdk(j,k),(msqvdk(j,k)-msqv(j,k))
!     &   /msq(j,k)/ason2pi
...
!        call gg_Hgaga_Hpi_v(p,msqv)
!        call gg_Hgaga_Hpi_z(p,z)
...
c--- for single top + b, make sure to use two different scales
          ...
          fx2z_L(:)=0._dp
          if (z > xx(1)) then
            call fdist_select(ih1,1,x1onz,facscale_H,fx1z_H)
            call fdist_select(ih1,1,x1onz,facscale_L,fx1z_L)
          endif
          if (z > xx(2)) then
            call fdist_select(ih2,2,x2onz,facscale_H,fx2z_H)
            call fdist_select(ih2,2,x2onz,facscale_L,fx2z_L)
          endif
        else
c--- usual case
          ...
          if (z > xx(1)) then
            call fdist_select(ih1,1,x1onz,facscale,fx1z)
          endif
          if (z > xx(2)) then
            call fdist_select(ih2,2,x2onz,facscale,fx2z)
          endif
        endif
...
c     &     +S2(g,g,g,gf_gf,cs,3)+S2(g,g,g,gf_gf,cs,2))*fx1(g)*fx2z(g)/z
     ...
     &   .or. (kcase==ktt_bbu) .or. (kcase==kgam_2j)
