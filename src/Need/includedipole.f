...
c--- implement cut on lead jet pt
!      if (jets == 1) then
!        if (pt(5,pjet) < ptleadjet) goto 999
!      endif
!      if (jets == 2) then
!        if (max(pt(5,pjet),pt(6,pjet)) < ptleadjet) goto 999
!      endif
!      if (jets == 3) then
!        if (max(pt(5,pjet),pt(6,pjet),pt(7,pjet)) < ptleadjet) goto 999
!      endif
...
!------ this branch is for inclusive studies, i.e. dirgam (inc Z,W,H etc.)
         elseif(scet_inc_jets) then
            call maketaucut_inc(ptrans,pincj,scetjets,isub,passed_taucut)
         
         elseif((kcase.eq.kZHbbnn).or.(kcase.eq.kZHbbjt).or.
     &           (kcase.eq.kZHn2ac)) then
!-----special branch for H=>bb decay at NNLO
!           call maketaucut_dec(ptrans,pjet,jets,isub,passed_taucut)
!            call jade_hbb(ptrans,npart,0.01_dp,jets,isub)
!            if (jets <=  4) passed_taucut=.false.
