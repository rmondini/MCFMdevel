...
c------ normal case
      ...
      coeffonly=.false.
      kpart=0
c--- default is to handle Wilson coefficient in-situ
      expansionorder=0
      Wilsonorder=0
     ...
     &   .or. (part == 'nnloRR') .or. (part == 'nnloRRcoeff') ) then
      ...
      elseif ((part == 'nnlowilson') .or. (part == 'nnloWilson')) then
        kpart=ktota
        expansionorder=2
      endif
...
c--- catch special scale choices for stop+b process
     ...
     &     (scale == 0._dp) .and. (facscale == 0._dp)) then
      ...
      elseif (index(line,'0.2%acc') > 0) then
        ...
        if ((line(1:2) == 'qt') .or. (line(1:2) == 'qT')
     & .or. (line(1:2) == 'Qt') .or. (line(1:2) == 'QT')) then
          doQT=.true.
          line=line(3:)
        elseif (line(1:7) == 'dynamic') then
          dynamictau=.true.
          line=line(8:)
        endif
        read(line,*) taucut
...
c--- jets and cuts options 
      ...
      read(20,*) Mwmax 
      if (Mwmax > sqrts) Mwmax=sqrts ! physical cap on m34max
      wsqmax=Mwmax**2
      ...
      read(20,*) mbbmax 
      if (mbbmax > sqrts) Mbbmax=sqrts ! physical cap on m56max
      bbsqmax=mbbmax**2
      ...
      read(20,*) ptleadjet
      if (verbose) call writeinput(6,' * ',' ','ptleadjet')
      read(20,*) etajetmin 
...
c--- settings for photon processes 
      ...
      call readrange(line,1,1.e6_dp,gammpt,gammptmax)
      if (verbose) call writeinput(6,' * ',' ','gammpt')
      read(20,'(a)') line
      if (index(line,',')>0) then
        read(line,*) gammrapmin,gammrap
      else
        gammrapmin=0d0
        read(line,*) gammrap
      endif
...
c--- determine whether SCET is to be used for calculation
      ...
      if     (index(runstring,'qqbonly') > 0) then
        selectpdfs(1)=+1
        selectpdfs(2)=-1
      elseif (index(runstring,'qbqonly') > 0) then
        selectpdfs(1)=-1
        selectpdfs(2)=+1
      elseif (index(runstring,'qqonly') > 0) then
        selectpdfs(1)=+1
        selectpdfs(2)=+1
      elseif (index(runstring,'qbqbonly') > 0) then
        selectpdfs(1)=-1
        selectpdfs(2)=-1
      elseif (index(runstring,'qgonly') > 0) then
        selectpdfs(1)=+1
        selectpdfs(2)=0
      elseif (index(runstring,'qbgonly') > 0) then
        selectpdfs(1)=-1
        selectpdfs(2)=0
      elseif (index(runstring,'gqonly') > 0) then
        selectpdfs(1)=0
        selectpdfs(2)=+1
      elseif (index(runstring,'gqbonly') > 0) then
        selectpdfs(1)=0
        selectpdfs(2)=-1
      elseif (index(runstring,'ggonly') > 0) then
        selectpdfs(1)=0
        selectpdfs(2)=0
      else
        selectpdfs(1:2)=99    ! dummy value with no effect
      endif
      
      if (index(runstring,'tauboost') > 0) then
        tauboost=.true.
      else
        tauboost=.false.
      endif
...
c--- this is an allowed combination
     ...
     &      .or. (kcase==kdirgam) .or. (kcase==kgamjet)
     &      .or. (kcase==kgmgmjt) .or .(kcase==ktrigam)
     &      .or. (kcase==kfourga) .or. (kcase==kdm_gam)
