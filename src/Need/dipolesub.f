...
c--- a canonical way; instead, pass emitted vector directly as "vec"
        if ( (kcase==kgamgam) .or. (kcase==kgg2gam)
     &  .or. (kcase==kHgagaI) ) then
          do nu=1,4
            vec(nu)=p(jp,nu)
          enddo
      endif
...
c--- do something special for gamma+2 jets
        if (kcase==kgam_2j) then
          if (jp .ne. 6) then
            if (ip < 6) then
C ie for cases 45_i,54_i
              ipt=4
            else
C ie for cases 65_i,64_i
              ipt=5
            endif
          else
C ie for cases 46_i,56_i
            ipt=ip
          endif
      endif      
        
c--- do something special for direct photon production
        if ((kcase==kdirgam) .or. (kcase==kgamjet)) ipt=4
