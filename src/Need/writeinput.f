...
c--- f95 2xfloating point format
      f95='('''//lstring//''',1x,f9.4,'','',f9.4,12x,''['',a,'']'','''
     & //rstring//''')' 
...
c--- catch special scale choices for stop+b process
     ...
     & lstring//' [Jet definition and event cuts] )'
      ...
      if ((tag == 'ptleadjet') .or. (writeall)) then
      write(unitno,fmt=f99) ptleadjet,'ptleadjet'
      endif
     ...
     & lstring//' [Settings for photon processes] )'
      ...
      write(unitno,fmt=f95) gammpt,gammptmax,'gammpt range'
      endif
      if ((tag == 'gammrap') .or. (writeall)) then
      write(unitno,fmt=f95) gammrapmin,gammrap,'gammrap range'
      endif
