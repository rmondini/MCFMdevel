...
c---  determine beginning of parton entries in plabel
      if (first) then
        first=.false.
        ipp=3
        do while ((ipp < mxpart ) .and. (.not.(is_hadronic(ipp))) .and. (plabel(ipp) .ne. ''))
          ipp=ipp+1
        enddo
        if (ipp == mxpart) then
...
c---  no more cuts if less than 4 partons in final state
      if (npart < i8-2) return

c--- reference direction for particle 8
      n8(:)=p(i8,:)/p(i8,4)

      t18=abs(n1(4)*p(i8,4)-n1(1)*p(i8,1)-n1(2)*p(i8,2)-n1(3)*p(i8,3))
      t28=abs(n2(4)*p(i8,4)-n2(1)*p(i8,1)-n2(2)*p(i8,2)-n2(3)*p(i8,3))

      if (p(i5,4) > p(i8,4)) then
        t58=n5(4)*p(i8,4)-n5(1)*p(i8,1)-n5(2)*p(i8,2)-n5(3)*p(i8,3)
      else
        t58=n8(4)*p(i5,4)-n8(1)*p(i5,1)-n8(2)*p(i5,2)-n8(3)*p(i5,3)
      endif

      if (p(i6,4) > p(i8,4)) then
        t68=n6(4)*p(i8,4)-n6(1)*p(i8,1)-n6(2)*p(i8,2)-n6(3)*p(i8,3)
      else
        t68=n8(4)*p(i6,4)-n8(1)*p(i6,1)-n8(2)*p(i6,2)-n8(3)*p(i6,3)
      endif
      if (p(i7,4) > p(i8,4)) then
        t78=n7(4)*p(i8,4)-n7(1)*p(i8,1)-n7(2)*p(i8,2)-n7(3)*p(i8,3)
      else
        t78=n8(4)*p(i7,4)-n8(1)*p(i7,1)-n8(2)*p(i7,2)-n8(3)*p(i7,3)
      endif

      
     
      if (t18 < cutoff) return 1
      if (t28 < cutoff) return 1
      if (t58 < cutoff) return 1
      if (t78 < cutoff) return 1
      if (t68 < cutoff) return 1
