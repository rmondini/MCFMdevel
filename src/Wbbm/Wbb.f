...
C     calculated in the Rodrigo scheme
     ...
     & p3Dq5,p4Dq5
      ...
      bp=0.5d0+0.5d0*beta
      bm=0.5d0-0.5d0*beta
      p3Dq5=+0.5d0*bp*s(q3,q5)+0.5d0*bm*s(q4,q5)
      p4Dq5=+0.5d0*bp*s(q4,q5)+0.5d0*bm*s(q3,q5)
...
C--This is qedi, for negative helicity gluon
     ...
     & /(zb(q5,q2)*zb(q5,q1)*s34)
      ...
      tmp(1,1)=2d0*m/zb(q3,q4)*helviol
      tmp(2,2)=2d0*m/za(q3,q4)*helviol
...
C---This is qedf, for negative helicity gluon
      tmp(1,2)=+(bp/p3Dq5-bm/p4Dq5)/(2d0*zb(q4,q5)*s345)
     ...
     & -m/(2d0*zb(q4,q5)*za(q4,q3)*p3Dq5*s167*s345)*(
     ...
     & -m/(2d0*zb(q4,q5)*zb(q4,q3)*p4Dq5*s167*s345)*(
     ...
     & -m/(2d0*zb(q4,q5)*zb(q4,q3)*p3Dq5*s267*s345)*(
     ...
     & -m/(2d0*zb(q4,q5)*zb(q4,q3)*p3Dq5*s167*s345)*(
...
c-- This is qcdb, for negative helicity gluon
     ...
     & /(zb(q4,q5)*s167*s345*s34)
      ...
      tmp(1,1)=
     & -m/(zb(q4,q5)*2d0*p3Dq5*s267*s345)*(
     & -zba4267*za(q4,q1)
     & +zba3267*za(q3,q1))*zb(q2,q6)*za(q3,q5)*bp

     & -m/(zb(q4,q3)*2d0*p3Dq5*s267*s345)*(
     &  -zba3267*zb(q2,q6)*za(q3,q5)*za(q1,q5))

     & -m/(zb(q4,q5)*2d0*p3Dq5*s167*s345)*(
     & +zb(q4,q2)*zba6174
     & -zb(q3,q2)*zba6173)*za(q3,q5)*za(q1,q7)*bp

     & -m/(zb(q4,q3)*2d0*p3Dq5*s167*s345)*(
     ...
     & -m/(2d0*zb(q4,q5)*s267*s345*s34)*(
     & -3d0*za(q4,q1)*zba4267+za(q3,q1)*zba3267
     & +za(q5,q1)*zba5267)*zb(q2,q6)*za(q3,q5)

     & -m/(2d0*zb(q4,q5)*s167*s345*s34)*(
     & +3d0*zb(q4,q2)*zba6174-zb(q3,q2)*zba6173
     ...
     & -m/(zb(q3,q5)*za(q4,q3)*p3Dq5*s267*s345*2d0)*(
     & +(zba4267*za(q4,q1)-zba3267*za(q3,q1))
     & *zb(q2,q6)*zb(q4,q3)*za(q4,q5)*bm)

     & -m/(zb(q3,q5)*za(q4,q3)*p3Dq5*s167*s345*2d0)*(
     & (zb(q3,q2)*zba6173-zb(q4,q2)*zba6174)
     ...
     & -m/(2d0*zb(q3,q5)*za(q4,q3)**2*s267*s345)*(
     & (zba4267*za(q1,q4)-3d0*zba3267*za(q1,q3)
     & +zba5267*za(q1,q5))*zb(q2,q6)*za(q4,q5))

     & -m/(2d0*zb(q3,q5)*za(q4,q3)**2*s167*s345)*(
     & (zb(q4,q2)*zba6174-3d0*zb(q3,q2)*zba6173
     & +zb(q5,q2)*zba6175)*za(q4,q5)*za(q1,q7))

     & -m/(za(q4,q3)*p3Dq5*s267*s345*2d0)*(
     & +zba4267*zb(q2,q6)*za(q4,q5)*za(q1,q5))

     & -m/(za(q4,q3)*p3Dq5*s167*s345*2d0)*(
     & +zb(q4,q2)*zba6175*za(q4,q5)*za(q1,q7))
