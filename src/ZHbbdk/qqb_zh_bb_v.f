...
!----- this is already present in MCFM in alternate form, but using same setup as NNLO stuff to test pieces
      
      subroutine qqb_zh_bb_v(p,msq)
      implicit none
      include 'types.f'
      include 'constants.f'
      include 'mxpart.f'
      ...
      include 'nflav.f'
      integer:: j,k
      real(dp):: s,prop,fac,q1423,q2413,v2(2)
...
!---- H=>bb decay @
      call Hbb_decay_v(p,5,6,hdecay_v)
      
!----- 2 body decay
      s56=s(5,6)
      
      hdecay=(hdecay_v)/((s56-hmass**2)**2+(hmass*hwidth)**2)
