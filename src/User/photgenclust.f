...
c---- Pick out jets 
 ...
 2    continue 
      ...
      pfinal(:,:)=0._dp
!---- pincj is for tau cuts if usescet, its the array of clustered
!---- partons, with no pt or eta requirements
      pincj(:,:)=0._dp
      scetjets=jets 
      do i=1,2
         do nu=1,4
...
c--- if no jets are removed by eta and pt cuts, then jets=ajet
       ...
       jets=ajet
!       if(jets.gt.1) then 
!       write(6,*) 'input momenta' 
!       call writeout(pin)
!       write(6,*) '*****************************************'
!       write(6,*) ' output momenta' 
!       call writeout(pincj) 
!       pause
!      endif
      return 
