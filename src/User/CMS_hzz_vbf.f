...
c--- ensure the tagging jets have an invariant mass larger than mjets
      m78=sqrt(max(zip,(p(7,4)+p(8,4))**2
...
c--- lepton rapidities between jets
      ...
      if ((etalep(3) < etajmin) .or. (etalep(3) > etajmax)) then
         failed_cuts=.true. 
         return 
      endif
      if ((etalep(4) < etajmin) .or. (etalep(4) > etajmax)) then
         failed_cuts=.true. 
         return 
      endif
      if ((etalep(5) < etajmin) .or. (etalep(5) > etajmax)) then
         failed_cuts=.true. 
         return 
      endif
      if ((etalep(6) < etajmin) .or. (etalep(6) > etajmax)) then
         failed_cuts=.true. 
         return 
      endif
