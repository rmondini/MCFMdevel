...
c--- APPLgrid - end
      ...
      logical atlasgamjet,cmsgamjet

!$omp threadprivate(plotindex)
      atlasgamjet=.false.
      cmsgamjet=.true.
      if (scet_inc_jets) then
        atlasgamjet=.false.
        cmsgamjet=.false.
      endif
...
c--- photon processes also need to know the dipole number
     ...
     &     .or. (kcase==kWgajet) .or. (kcase==kZgajet)) then
        ...
        elseif (kcase==kgam_2j) then
          if (usescet) then  ! same routine below and above cut for SCET
            if(atlasgamjet) then
               plotindex=130
            elseif(cmsgamjet) then
               plotindex=32
            else
               plotindex=13
            endif
     ...
     &          .or. (kcase==kZ_3jet)) then
      ...
      case (130)
        call nplotter_gamjet(p,wt,wt2,switch,nd)
      case (14)
