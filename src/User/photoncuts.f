...
!---- optional cut of y3 for ATLAS study
!       if(abs(etarap(3,pjet)) < 1.81_dp) then 
!          photoncuts=.true.
!          return
!       endif
...
!$omp master
     ...
     &                '                *'
        ...
        write(6,97) gammpt,'    pt(photon)     ',gammptmax,'GeV'
      endif
     ...
     &        '                *'
      ...
      write(6,97) gammrapmin,'   |eta(photon)|   ',gammrap,'   '
...
C     Basic pt and rapidity cuts for photon
      if (countgamm == 1) then
          ...
          if ( (pt1 < gammpt) .or. (pt1 > gammptmax) .or.
     &    (eta1 > gammrap) .or. (eta1 < gammrapmin)) then
            photoncuts=.true.
            return
          endif
      endif
      if (countgamm == 2) then
        ...
        pth=max(pt1,pt2)
        pts=min(pt1,pt2)
        if ( ( pth < gammpt) .or. (pth > gammptmax) .or. 
     &       ( pts < gammpt2) .or.
     &       (eta1 > gammrap) .or. (eta1 < gammrapmin) .or.
     &       (eta2 > gammrap) .or. (eta2 < gammrapmin) ) then
          photoncuts=.true.
          return
        endif
      endif
     ...
     &       ( pts < gammpt3) .or.
     &       (eta1 > gammrap) .or. (eta1 < gammrapmin) .or.
     &       (eta2 > gammrap) .or. (eta2 < gammrapmin) .or.
     &       (eta3 > gammrap) .or. (eta3 < gammrapmin) ) then
          photoncuts=.true.
          return
        endif
      endif
     ...
     &       (eta1 > gammrap) .or. (eta1 < gammrapmin) .or.
     &       (eta2 > gammrap) .or. (eta2 < gammrapmin) .or.
     &       (eta3 > gammrap) .or. (eta3 < gammrapmin) .or.
     &       (eta4 > gammrap) .or. (eta4 < gammrapmin) ) then
          photoncuts=.true.
          return
        endif
      endif
