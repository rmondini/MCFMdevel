...
!----- paramaters eps, delta_0 and n_pow are taken from input.DAT 
      ...
      include 'first.f'
      include 'mpicommon.f'
      real(dp):: p(mxpart,4),R,pref,ret_ET,cone_size,ET_gam
      integer:: j,isub,k
...
!----- check for non-zero parameters, if zero exit with warning 
        if((epsilon_h<vsmall).or.(abs(cone_ang)<vsmall)) then 
!$omp master
          if (rank == 0) then
          write(6,*)
...
!$omp master
        ...
        write(6,*) '************** Frixione Isolation    ***************'
        write(6,*) '*                                                  *'
        if (cone_ang < 0._dp) then
          write(6,96)'*  eps_phot = ',epsilon_h,'                *'
          write(6,98)'*  delta_0 = min[',-cone_ang,
     &      ', mZ/ET(gam)/sqrt(eps_phot)] *'
        else
          write(6,99)'*  eps_phot = ',epsilon_h,', delta_0 = ',cone_ang,  '*'
        endif
        write(6,97)'*  n = ',n_pow,'                                   *'
...
!$omp end master
 ...
 98   format (1x,a17,f5.2,a30)
 97   format (1x,a7,f5.2,a40)
 96   format (1x,a14,f5.3,a33)
...
!----- else fail and return 
      ...
      if (cone_ang > 0._dp) then
! standard fixed cone-size
        cone_size=cone_ang
! dynamic cone-size of Pozzorini et al. DM study
      else
        cone_size=min(-cone_ang,zmass/ET_gam/sqrt(epsilon_h))
      endif
      
      pref=ET_gam*epsilon_h/((1._dp-cos(cone_size))**n_pow)
!      pref=epsilon_h/((1._dp-cos(cone_size))**n_pow)
      ET_had=0._dp
...
!======first thing, find a hadron inside isolation cone
         if(is_hadronic(i).and.(R(p,i,j)<cone_size)) then 
