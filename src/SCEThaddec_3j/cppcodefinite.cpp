...
extern "C" double wrpfinite(double y, double z, double logm){

	string textread,fline1,fline2;
...
// form factors at LO with overall factor sqrt(as/(2*pi))
	ex a1born=2*sqrt(2)*Pi*(1.0/yy+1.0/zz);
	ex a2born=2*sqrt(2)*Pi*(-2.0/yy);
...
//	yy=-0.18;
//	zz=1.45;
//	ex fline3 = -G(lst(1,0,zz),-xx);
//	ex fline4 = evalf(fline3);
//    cout << "prova = " << fline4 << endl;
...
//	ifstream inputfile1("../src/SCEThaddec_3j/a1nnlofinite.txt");
	ifstream inputfile1("../src/SCEThaddec_3j/a1nnlofinitelitreslog1.txt");
    while (getline(inputfile1, textread))
    {
       	fline1 += textread;
    }

//	ifstream inputfile2("../src/SCEThaddec_3j/a2nnlofinite.txt");
	ifstream inputfile2("../src/SCEThaddec_3j/a2nnlofinitelitreslog1.txt");
    while (getline(inputfile2, textread))
    {
       	fline2 += textread;
    }

	ex a1finite = reader_table(fline1).evalf();
	ex a2finite = reader_table(fline2).evalf();

    cout << "a1finite = " << a1finite << endl;
    cout << "a2finite = " << a2finite << endl;

// construction of all required pieces of the interference

	ex finitepartoneonenoeps=4.0*yy*zz*a1finite*a1born;
	ex finiteparttwotwo=2.0*pow(xx,2)*yy/zz*a2finite*a2born;
	ex finitepartonetwo=-2.0*xx*yy*a1finite*a2born;
	ex finiteparttwoone=-2.0*xx*yy*a2finite*a1born;

	ex finitepartsum=evalf(2.0*(finitepartoneonenoeps+finiteparttwotwo+finitepartonetwo+finiteparttwoone));

	return ex_to<numeric>(finitepartsum).to_double();
