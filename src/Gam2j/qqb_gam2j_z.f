      subroutine qqb_gam2j_z(p,z)
      implicit none
      include 'types.f'
!     CW----- this is copy of the Z+2j version
!------ changes 5=>4 6=>5 
************************************************************************
*     Author: J.M. Campbell                                            *
*     September, 2001.                                                 *
...
************************************************************************
     ...
     &                 ff_qq,ff_gg,ff_gq,fi_gq
      ...
      integer:: is
       ...
       xl15=log(-two*dot(p,1,4)/musq)
       xl16=log(-two*dot(p,1,5)/musq)
       xl25=log(-two*dot(p,2,4)/musq)
       xl26=log(-two*dot(p,2,5)/musq)
       xl56=log(+two*dot(p,4,5)/musq)

!     xl12=log(+two*dot(p,1,2)/musq)
!       xl15=log(-two*dot(p,1,5)/musq)
!       xl16=log(-two*dot(p,1,4)/musq)
!       xl25=log(-two*dot(p,2,5)/musq)
!       xl26=log(-two*dot(p,2,4)/musq)
!       xl56=log(+two*dot(p,4,5)/musq)
